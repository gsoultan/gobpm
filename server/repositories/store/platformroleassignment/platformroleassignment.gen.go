...
// source: table platform_role_assignments

package platformroleassignment
...
type Row struct {
	PlatformUserID [16]byte
	PlatformRoleID [16]byte
}
...
var (
	PlatformUserID = UUIDCol{0}
	PlatformRoleID = UUIDCol{1}
)
...
func (q Query) PlatformUserIDEq(v [16]byte) Query       { return q.Where(PlatformUserID.Eq(v)) }
func (q Query) PlatformUserIDNotEq(v [16]byte) Query    { return q.Where(PlatformUserID.NotEq(v)) }
func (q Query) PlatformUserIDIn(v ...[16]byte) Query    { return q.Where(PlatformUserID.In(v...)) }
func (q Query) PlatformUserIDNotIn(v ...[16]byte) Query { return q.Where(PlatformUserID.NotIn(v...)) }
func (q Query) PlatformRoleIDEq(v [16]byte) Query       { return q.Where(PlatformRoleID.Eq(v)) }
func (q Query) PlatformRoleIDNotEq(v [16]byte) Query    { return q.Where(PlatformRoleID.NotEq(v)) }
func (q Query) PlatformRoleIDIn(v ...[16]byte) Query    { return q.Where(PlatformRoleID.In(v...)) }
func (q Query) PlatformRoleIDNotIn(v ...[16]byte) Query { return q.Where(PlatformRoleID.NotIn(v...)) }

const selectPrefix = `SELECT "platform_user_id", "platform_role_id" FROM "platform_role_assignments"`
const countPrefix = `SELECT count(*) FROM "platform_role_assignments"`
const existsPrefix = `SELECT 1 FROM "platform_role_assignments"`
...
var orderTable = [nCols][4]string{
	{ // platform_user_id
		...
		"\"platform_user_id\" ASC NULLS FIRST",
		"\"platform_user_id\" DESC NULLS LAST",
	},
	{ // platform_role_id
		"\"platform_role_id\"",
		"\"platform_role_id\" DESC",
		"\"platform_role_id\" ASC NULLS FIRST",
		"\"platform_role_id\" DESC NULLS LAST",
	},
}
...
var fragTable = [2][27]runtime.Frag{
	{ // platform_user_id
		...
		{},
		{},
		{A: "\"platform_user_id\" = ANY($", B: ")"},
		{A: "\"platform_user_id\" <> ALL($", B: ")"},
		{},
		{},
		{},
		{},
	...
	{ // platform_role_id
		...
		{},
		{},
		{A: "\"platform_role_id\" = ANY($", B: ")"},
		{A: "\"platform_role_id\" <> ALL($", B: ")"},
		{},
		{},
		{},
		{},
...
var defaultOrder = [2]runtime.Tok{
	runtime.MakeOrder(runtime.Asc, 0), // platform_user_id
	runtime.MakeOrder(runtime.Asc, 1), // platform_role_id
}
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	copy(r.PlatformUserID[:], rv[0])
	copy(r.PlatformRoleID[:], rv[1])
	return nil
}
...
// BatchTopByPlatformUserID fetches at most n rows for each id, ordered by order.
//
...
func BatchTopByPlatformUserID(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return BatchTopByPlatformUserIDLateral(ctx, ex, ids, n, order...)
}

// BatchTopByPlatformUserIDWindow runs the window lowering.
//
...
// involve writing SQL.
func BatchTopByPlatformUserIDWindow(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByPlatformUserIDRun(ctx, ex, batchTopByPlatformUserIDWindowSQL(order), ids, n)
}

// BatchTopByPlatformUserIDLateral runs the lateral lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByPlatformUserIDLateral(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByPlatformUserIDRun(ctx, ex, batchTopByPlatformUserIDLateralSQL(order), ids, n)
}

// batchTopByPlatformUserIDRun executes one lowered statement.
func batchTopByPlatformUserIDRun(ctx context.Context, ex runtime.Executor, sql string, ids [][16]byte, n int64) ([]Row, error) {
	if len(ids) == 0 || n <= 0 {
		return nil, nil
	}
	if sql == "" {
...
var batchTopByPlatformUserIDWindowCache = runtime.NewTreeCache()

// batchTopByPlatformUserIDWindowSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByPlatformUserIDWindowSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	...
	if st := batchTopByPlatformUserIDWindowCache.Get(toks); st != nil {
		return st.SQL
	}
	...
	sql := runtime.SpliceOrder("SELECT \"platform_user_id\", \"platform_role_id\" FROM (SELECT \"platform_user_id\", \"platform_role_id\", row_number() OVER (PARTITION BY \"platform_user_id\"\x00order\x00) AS \"_storm_rn\" FROM \"platform_role_assignments\" WHERE \"platform_user_id\" = ANY($1)) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
	return batchTopByPlatformUserIDWindowCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

var batchTopByPlatformUserIDLateralCache = runtime.NewTreeCache()

// batchTopByPlatformUserIDLateralSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByPlatformUserIDLateralSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByPlatformUserIDLateralCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"platform_user_id\", \"_storm_c\".\"platform_role_id\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"platform_user_id\", \"platform_role_id\" FROM \"platform_role_assignments\" WHERE \"platform_user_id\" = \"_storm_p\".\"_storm_k\"\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
	return batchTopByPlatformUserIDLateralCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}
...
const insertSQL = `INSERT INTO "platform_role_assignments" ("platform_user_id", "platform_role_id") VALUES ($1, $2) RETURNING "platform_user_id", "platform_role_id"`

const updatePrefix = `UPDATE "platform_role_assignments" SET `
const deletePrefix = `DELETE FROM "platform_role_assignments"`
...
var pkFrags = [2]runtime.Frag{
	{A: "\"platform_user_id\" = $", B: ""}, // platform_user_id
	{A: "\"platform_role_id\" = $", B: ""}, // platform_role_id
}
...
const (
	iPlatformUserID uint64 = 1 << 0
	iPlatformRoleID uint64 = 1 << 1
)
...
const insPrefix = "INSERT INTO \"platform_role_assignments\""
const insReturning = " RETURNING \"platform_user_id\", \"platform_role_id\""
...
func (n *Ins) SetPlatformUserID(v [16]byte) {
	n.row.PlatformUserID = v
	n.set |= iPlatformUserID
}

func (n *Ins) SetPlatformRoleID(v [16]byte) {
	n.row.PlatformRoleID = v
	n.set |= iPlatformRoleID
}
...
var conflictSpecs = []string{
	" ON CONFLICT (\"platform_user_id\", \"platform_role_id\")",
}
...
// OnConflictPlatformUserIDPlatformRoleID upserts on the unique index over (platform_user_id, platform_role_id).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictPlatformUserIDPlatformRoleID() *Ins {
	n.conflict = 2
	return n
}
...
//	n.OnConflictPlatformUserIDPlatformRoleID().DoNothing()  // only on that one index
//
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		switch i {
		case 0:
			args = append(args, n.row.PlatformUserID)
		case 1:
			args = append(args, n.row.PlatformRoleID)
		}
	}
	var out Row
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	args := make([]any, 0, 2)
	args = append(args, r.PlatformUserID)
	args = append(args, r.PlatformRoleID)
...
func (s *rowSource) Values() []any {
	r := &s.rows[s.i-1]
	s.buf[0] = &r.PlatformUserID
	s.buf[1] = &r.PlatformRoleID
	return s.buf[:]
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "platform_role_assignments", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	args = append(args, m.row.PlatformUserID)
	args = append(args, m.row.PlatformRoleID)
	return runtime.BatchOp{SQL: st.SQL, Args: args}, true
}

// DeleteOp is a delete as a queueable statement.
func DeleteOp(platformUserID [16]byte, platformRoleID [16]byte) runtime.BatchOp {
	return runtime.BatchOp{SQL: deleteSQL, Args: []any{platformUserID, platformRoleID}}
}
...
const Table = "platform_role_assignments"
...
func Delete(ctx context.Context, ex runtime.Executor, platformUserID [16]byte, platformRoleID [16]byte) error {
	n, err := ex.Exec(ctx, deleteSQL, []any{platformUserID, platformRoleID})
	if err != nil {
		return err
	}
	if n == 0 {
