...
// source: table workflow_group_memberships

package workflowgroupmembership
...
type Row struct {
	WorkflowUserID  [16]byte
	WorkflowGroupID [16]byte
}
...
var (
	WorkflowUserID  = UUIDCol{0}
	WorkflowGroupID = UUIDCol{1}
)
...
func (q Query) WorkflowUserIDEq(v [16]byte) Query        { return q.Where(WorkflowUserID.Eq(v)) }
func (q Query) WorkflowUserIDNotEq(v [16]byte) Query     { return q.Where(WorkflowUserID.NotEq(v)) }
func (q Query) WorkflowUserIDIn(v ...[16]byte) Query     { return q.Where(WorkflowUserID.In(v...)) }
func (q Query) WorkflowUserIDNotIn(v ...[16]byte) Query  { return q.Where(WorkflowUserID.NotIn(v...)) }
func (q Query) WorkflowGroupIDEq(v [16]byte) Query       { return q.Where(WorkflowGroupID.Eq(v)) }
func (q Query) WorkflowGroupIDNotEq(v [16]byte) Query    { return q.Where(WorkflowGroupID.NotEq(v)) }
func (q Query) WorkflowGroupIDIn(v ...[16]byte) Query    { return q.Where(WorkflowGroupID.In(v...)) }
func (q Query) WorkflowGroupIDNotIn(v ...[16]byte) Query { return q.Where(WorkflowGroupID.NotIn(v...)) }

const selectPrefix = `SELECT "workflow_user_id", "workflow_group_id" FROM "workflow_group_memberships"`
const countPrefix = `SELECT count(*) FROM "workflow_group_memberships"`
const existsPrefix = `SELECT 1 FROM "workflow_group_memberships"`
...
var orderTable = [nCols][4]string{
	{ // workflow_user_id
		"\"workflow_user_id\"",
		"\"workflow_user_id\" DESC",
		"\"workflow_user_id\" ASC NULLS FIRST",
		"\"workflow_user_id\" DESC NULLS LAST",
	},
	{ // workflow_group_id
		"\"workflow_group_id\"",
		"\"workflow_group_id\" DESC",
		"\"workflow_group_id\" ASC NULLS FIRST",
		"\"workflow_group_id\" DESC NULLS LAST",
	},
}
...
var fragTable = [2][27]runtime.Frag{
	{ // workflow_user_id
		...
		{},
		{},
		{A: "\"workflow_user_id\" = ANY($", B: ")"},
		{A: "\"workflow_user_id\" <> ALL($", B: ")"},
		{},
		{},
		{},
		{},
	...
	{ // workflow_group_id
		...
		{},
		{},
		{A: "\"workflow_group_id\" = ANY($", B: ")"},
		{A: "\"workflow_group_id\" <> ALL($", B: ")"},
		{},
		{},
		{},
		{},
...
var defaultOrder = [2]runtime.Tok{
	runtime.MakeOrder(runtime.Asc, 0), // workflow_user_id
	runtime.MakeOrder(runtime.Asc, 1), // workflow_group_id
}
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	copy(r.WorkflowUserID[:], rv[0])
	copy(r.WorkflowGroupID[:], rv[1])
	return nil
}
...
// BatchTopByWorkflowGroupID fetches at most n rows for each id, ordered by order.
//
...
func BatchTopByWorkflowGroupID(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return BatchTopByWorkflowGroupIDLateral(ctx, ex, ids, n, order...)
}

// BatchTopByWorkflowGroupIDWindow runs the window lowering.
//
...
// involve writing SQL.
func BatchTopByWorkflowGroupIDWindow(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByWorkflowGroupIDRun(ctx, ex, batchTopByWorkflowGroupIDWindowSQL(order), ids, n)
}

// BatchTopByWorkflowGroupIDLateral runs the lateral lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByWorkflowGroupIDLateral(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByWorkflowGroupIDRun(ctx, ex, batchTopByWorkflowGroupIDLateralSQL(order), ids, n)
}

// batchTopByWorkflowGroupIDRun executes one lowered statement.
func batchTopByWorkflowGroupIDRun(ctx context.Context, ex runtime.Executor, sql string, ids [][16]byte, n int64) ([]Row, error) {
	if len(ids) == 0 || n <= 0 {
		return nil, nil
	}
	if sql == "" {
...
var batchTopByWorkflowGroupIDWindowCache = runtime.NewTreeCache()

// batchTopByWorkflowGroupIDWindowSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByWorkflowGroupIDWindowSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	...
	if st := batchTopByWorkflowGroupIDWindowCache.Get(toks); st != nil {
		return st.SQL
	}
	...
	sql := runtime.SpliceOrder("SELECT \"workflow_user_id\", \"workflow_group_id\" FROM (SELECT \"workflow_user_id\", \"workflow_group_id\", row_number() OVER (PARTITION BY \"workflow_group_id\"\x00order\x00) AS \"_storm_rn\" FROM \"workflow_group_memberships\" WHERE \"workflow_group_id\" = ANY($1)) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
	return batchTopByWorkflowGroupIDWindowCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

var batchTopByWorkflowGroupIDLateralCache = runtime.NewTreeCache()

// batchTopByWorkflowGroupIDLateralSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByWorkflowGroupIDLateralSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByWorkflowGroupIDLateralCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"workflow_user_id\", \"_storm_c\".\"workflow_group_id\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"workflow_user_id\", \"workflow_group_id\" FROM \"workflow_group_memberships\" WHERE \"workflow_group_id\" = \"_storm_p\".\"_storm_k\"\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
	return batchTopByWorkflowGroupIDLateralCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

// BatchTopByWorkflowUserID fetches at most n rows for each id, ordered by order.
//
// ONE round trip regardless of how many ids are passed — the per-parent
// limit is expressed in SQL, not by looping. It delegates to the lowering
// chosen at BUILD time by measurement (bench/RESULTS.md); the strategy is
// never sniffed at run time.
func BatchTopByWorkflowUserID(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return BatchTopByWorkflowUserIDLateral(ctx, ex, ids, n, order...)
}

// BatchTopByWorkflowUserIDWindow runs the window lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByWorkflowUserIDWindow(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByWorkflowUserIDRun(ctx, ex, batchTopByWorkflowUserIDWindowSQL(order), ids, n)
}

// BatchTopByWorkflowUserIDLateral runs the lateral lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByWorkflowUserIDLateral(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByWorkflowUserIDRun(ctx, ex, batchTopByWorkflowUserIDLateralSQL(order), ids, n)
}

// batchTopByWorkflowUserIDRun executes one lowered statement.
func batchTopByWorkflowUserIDRun(ctx context.Context, ex runtime.Executor, sql string, ids [][16]byte, n int64) ([]Row, error) {
	if len(ids) == 0 || n <= 0 {
		return nil, nil
	}
	if sql == "" {
...
var batchTopByWorkflowUserIDWindowCache = runtime.NewTreeCache()

// batchTopByWorkflowUserIDWindowSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByWorkflowUserIDWindowSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByWorkflowUserIDWindowCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
	...
	sql := runtime.SpliceOrder("SELECT \"workflow_user_id\", \"workflow_group_id\" FROM (SELECT \"workflow_user_id\", \"workflow_group_id\", row_number() OVER (PARTITION BY \"workflow_user_id\"\x00order\x00) AS \"_storm_rn\" FROM \"workflow_group_memberships\" WHERE \"workflow_user_id\" = ANY($1)) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
	return batchTopByWorkflowUserIDWindowCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

var batchTopByWorkflowUserIDLateralCache = runtime.NewTreeCache()

// batchTopByWorkflowUserIDLateralSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByWorkflowUserIDLateralSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByWorkflowUserIDLateralCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"workflow_user_id\", \"_storm_c\".\"workflow_group_id\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"workflow_user_id\", \"workflow_group_id\" FROM \"workflow_group_memberships\" WHERE \"workflow_user_id\" = \"_storm_p\".\"_storm_k\"\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
	return batchTopByWorkflowUserIDLateralCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}
...
const insertSQL = `INSERT INTO "workflow_group_memberships" ("workflow_user_id", "workflow_group_id") VALUES ($1, $2) RETURNING "workflow_user_id", "workflow_group_id"`

const updatePrefix = `UPDATE "workflow_group_memberships" SET `
const deletePrefix = `DELETE FROM "workflow_group_memberships"`
...
var pkFrags = [2]runtime.Frag{
	{A: "\"workflow_user_id\" = $", B: ""},  // workflow_user_id
	{A: "\"workflow_group_id\" = $", B: ""}, // workflow_group_id
}
...
const (
	iWorkflowUserID  uint64 = 1 << 0
	iWorkflowGroupID uint64 = 1 << 1
)
...
const insPrefix = "INSERT INTO \"workflow_group_memberships\""
const insReturning = " RETURNING \"workflow_user_id\", \"workflow_group_id\""
...
func (n *Ins) SetWorkflowUserID(v [16]byte) {
	n.row.WorkflowUserID = v
	n.set |= iWorkflowUserID
}

func (n *Ins) SetWorkflowGroupID(v [16]byte) {
	n.row.WorkflowGroupID = v
	n.set |= iWorkflowGroupID
}
...
var conflictSpecs = []string{
	" ON CONFLICT (\"workflow_user_id\", \"workflow_group_id\")",
}
...
// OnConflictWorkflowUserIDWorkflowGroupID upserts on the unique index over (workflow_user_id, workflow_group_id).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictWorkflowUserIDWorkflowGroupID() *Ins {
	n.conflict = 2
	return n
}
...
//	n.OnConflictWorkflowUserIDWorkflowGroupID().DoNothing()  // only on that one index
//
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		switch i {
		case 0:
			args = append(args, n.row.WorkflowUserID)
		case 1:
			args = append(args, n.row.WorkflowGroupID)
		}
	}
	var out Row
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	args := make([]any, 0, 2)
	args = append(args, r.WorkflowUserID)
	args = append(args, r.WorkflowGroupID)
...
func (s *rowSource) Values() []any {
	r := &s.rows[s.i-1]
	s.buf[0] = &r.WorkflowUserID
	s.buf[1] = &r.WorkflowGroupID
	return s.buf[:]
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "workflow_group_memberships", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	args = append(args, m.row.WorkflowUserID)
	args = append(args, m.row.WorkflowGroupID)
	return runtime.BatchOp{SQL: st.SQL, Args: args}, true
}

// DeleteOp is a delete as a queueable statement.
func DeleteOp(workflowUserID [16]byte, workflowGroupID [16]byte) runtime.BatchOp {
	return runtime.BatchOp{SQL: deleteSQL, Args: []any{workflowUserID, workflowGroupID}}
}
...
const Table = "workflow_group_memberships"
...
func Delete(ctx context.Context, ex runtime.Executor, workflowUserID [16]byte, workflowGroupID [16]byte) error {
	n, err := ex.Exec(ctx, deleteSQL, []any{workflowUserID, workflowGroupID})
	if err != nil {
		return err
	}
	if n == 0 {
