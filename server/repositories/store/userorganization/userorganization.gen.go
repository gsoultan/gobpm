...
// source: table user_organizations
...
var defaultOrder = [2]runtime.Tok{
	...
	runtime.MakeOrder(runtime.Asc, 1), // organization_id
}
...
// OnConflictUserIDOrganizationID upserts on the unique index over (user_id, organization_id).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictUserIDOrganizationID() *Ins {
	n.conflict = 2
	return n
}
...
//	n.OnConflictUserIDOrganizationID().DoNothing()  // only on that one index
//
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "user_organizations", copyCols, src)
}
...
func DeleteOp(userID [16]byte, organizationID [16]byte) runtime.BatchOp {
	return runtime.BatchOp{SQL: deleteSQL, Args: []any{userID, organizationID}}
}
...
const Table = "user_organizations"
...
func Delete(ctx context.Context, ex runtime.Executor, userID [16]byte, organizationID [16]byte) error {
	n, err := ex.Exec(ctx, deleteSQL, []any{userID, organizationID})
	if err != nil {
		return err
	}
	if n == 0 {
