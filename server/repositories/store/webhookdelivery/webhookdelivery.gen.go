...
var (
	...
	UpdatedAt  = TimeCol{2}
	WebhookID  = UUIDCol{3}
	DeliveryID = TextCol{4}
	ReceivedAt = TimeCol{5}
...
func (q Query) WebhookIDEq(v [16]byte) Query       { return q.Where(WebhookID.Eq(v)) }
func (q Query) WebhookIDNotEq(v [16]byte) Query    { return q.Where(WebhookID.NotEq(v)) }
func (q Query) WebhookIDIn(v ...[16]byte) Query    { return q.Where(WebhookID.In(v...)) }
func (q Query) WebhookIDNotIn(v ...[16]byte) Query { return q.Where(WebhookID.NotIn(v...)) }
func (q Query) DeliveryIDEq(v string) Query        { return q.Where(DeliveryID.Eq(v)) }
func (q Query) DeliveryIDNotEq(v string) Query     { return q.Where(DeliveryID.NotEq(v)) }
func (q Query) DeliveryIDGt(v string) Query        { return q.Where(DeliveryID.Gt(v)) }
func (q Query) DeliveryIDGte(v string) Query       { return q.Where(DeliveryID.Gte(v)) }
func (q Query) DeliveryIDLt(v string) Query        { return q.Where(DeliveryID.Lt(v)) }
func (q Query) DeliveryIDLte(v string) Query       { return q.Where(DeliveryID.Lte(v)) }
func (q Query) DeliveryIDLike(v string) Query      { return q.Where(DeliveryID.Like(v)) }
func (q Query) DeliveryIDILike(v string) Query     { return q.Where(DeliveryID.ILike(v)) }
func (q Query) DeliveryIDIn(v ...string) Query     { return q.Where(DeliveryID.In(v...)) }
func (q Query) DeliveryIDNotIn(v ...string) Query  { return q.Where(DeliveryID.NotIn(v...)) }
func (q Query) ReceivedAtEq(v time.Time) Query     { return q.Where(ReceivedAt.Eq(v)) }
func (q Query) ReceivedAtNotEq(v time.Time) Query  { return q.Where(ReceivedAt.NotEq(v)) }
func (q Query) ReceivedAtGt(v time.Time) Query     { return q.Where(ReceivedAt.Gt(v)) }
func (q Query) ReceivedAtGte(v time.Time) Query    { return q.Where(ReceivedAt.Gte(v)) }
func (q Query) ReceivedAtLt(v time.Time) Query     { return q.Where(ReceivedAt.Lt(v)) }
func (q Query) ReceivedAtLte(v time.Time) Query    { return q.Where(ReceivedAt.Lte(v)) }
...
const selectPrefix = `SELECT "id", "created_at", "updated_at", "webhook_id", "delivery_id", "received_at", "deleted_at" FROM "webhook_deliveries"`
const countPrefix = `SELECT count(*) FROM "webhook_deliveries"`
const existsPrefix = `SELECT 1 FROM "webhook_deliveries"`
...
var orderTable = [nCols][4]string{
	...
	{ // webhook_id
		"\"webhook_id\"",
		"\"webhook_id\" DESC",
		"\"webhook_id\" ASC NULLS FIRST",
		"\"webhook_id\" DESC NULLS LAST",
	},
	{ // delivery_id
		"\"delivery_id\"",
		"\"delivery_id\" DESC",
		"\"delivery_id\" ASC NULLS FIRST",
		"\"delivery_id\" DESC NULLS LAST",
	},
	{ // received_at
		"\"received_at\"",
		"\"received_at\" DESC",
		"\"received_at\" ASC NULLS FIRST",
...
var fragTable = [7][27]runtime.Frag{
	...
	{ // delivery_id
		...
		{A: "\"delivery_id\" < $", B: ""},
		{A: "\"delivery_id\" <= $", B: ""},
		{A: "\"delivery_id\" LIKE $", B: ""},
		{A: "\"delivery_id\" ILIKE $", B: ""},
		{},
		{},
		{},
		{},
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	copy(r.WebhookID[:], rv[3])
	r.DeliveryID = sl.Str(rv[4])
	r.ReceivedAt = runtime.Timestamptz(rv[5])
...
const insertSQL = `INSERT INTO "webhook_deliveries" ("id", "created_at", "updated_at", "webhook_id", "delivery_id", "received_at", "deleted_at") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING "id", "created_at", "updated_at", "webhook_id", "delivery_id", "received_at", "deleted_at"`

const updatePrefix = `UPDATE "webhook_deliveries" SET `
const deletePrefix = `DELETE FROM "webhook_deliveries"`
...
const (
	...
	dWebhookID  uint64 = 1 << 1
	dDeliveryID uint64 = 1 << 2
	dReceivedAt uint64 = 1 << 3
...
const (
	...
	iWebhookID  uint64 = 1 << 3
	iDeliveryID uint64 = 1 << 4
	iReceivedAt uint64 = 1 << 5
...
const insPrefix = "INSERT INTO \"webhook_deliveries\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"webhook_id\", \"delivery_id\", \"received_at\", \"deleted_at\""
...
func (m *Mut) SetWebhookID(v [16]byte) {
	m.row.WebhookID = v
	m.dirty |= dWebhookID
}

func (m *Mut) SetDeliveryID(v string) {
	m.row.DeliveryID = v
	m.dirty |= dDeliveryID
}

func (m *Mut) SetReceivedAt(v time.Time) {
	m.row.ReceivedAt = v
	m.dirty |= dReceivedAt
}
...
func (n *Ins) SetWebhookID(v [16]byte) {
	n.row.WebhookID = v
	n.set |= iWebhookID
}

func (n *Ins) SetDeliveryID(v string) {
	n.row.DeliveryID = v
	n.set |= iDeliveryID
}

func (n *Ins) SetReceivedAt(v time.Time) {
	n.row.ReceivedAt = v
	n.set |= iReceivedAt
}
...
var conflictSpecs = []string{
	" ON CONFLICT (\"id\")",
	" ON CONFLICT (\"webhook_id\", \"delivery_id\")",
}
...
func assignable(i uint8, mask uint64) []string {
	...
	case 0:
		...
		if mask&(1<<3) != 0 {
			set = append(set, "webhook_id")
		}
		if mask&(1<<4) != 0 {
			set = append(set, "delivery_id")
		}
		if mask&(1<<5) != 0 {
			set = append(set, "received_at")
		}
		if mask&(1<<6) != 0 {
...
// OnConflictWebhookIDDeliveryID upserts on the unique index over (webhook_id, delivery_id).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictWebhookIDDeliveryID() *Ins {
	n.conflict = 4
	return n
}
...
var assignFor = map[string]string{
	...
	"webhook_id":  "\"webhook_id\" = EXCLUDED.\"webhook_id\"",
	"delivery_id": "\"delivery_id\" = EXCLUDED.\"delivery_id\"",
	"received_at": "\"received_at\" = EXCLUDED.\"received_at\"",
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 3:
			args = append(args, n.row.WebhookID)
		case 4:
			args = append(args, n.row.DeliveryID)
		case 5:
			args = append(args, n.row.ReceivedAt)
		case 6:
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	...
	args = append(args, r.WebhookID)
	args = append(args, r.DeliveryID)
	args = append(args, r.ReceivedAt)
...
func (s *rowSource) Values() []any {
	...
	s.buf[2] = &r.UpdatedAt
	s.buf[3] = &r.WebhookID
	s.buf[4] = &r.DeliveryID
	s.buf[5] = &r.ReceivedAt
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "webhook_deliveries", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 1:
			args = append(args, m.row.WebhookID)
		case 2:
			args = append(args, m.row.DeliveryID)
		case 3:
			args = append(args, m.row.ReceivedAt)
		case 4:
...
var softDeleteSQL = runtime.SpliceSections(`UPDATE "webhook_deliveries" SET "deleted_at" = now()`, []runtime.Section{
...
var restoreSQL = runtime.SpliceSections(`UPDATE "webhook_deliveries" SET "deleted_at" = NULL`, []runtime.Section{
