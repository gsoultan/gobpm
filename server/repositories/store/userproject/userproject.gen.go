...
const selectPrefix = `SELECT "user_id", "project_id" FROM "user_projects"`
const countPrefix = `SELECT count(*) FROM "user_projects"`
const existsPrefix = `SELECT 1 FROM "user_projects"`
...
var defaultOrder = [2]runtime.Tok{
	...
	runtime.MakeOrder(runtime.Asc, 1), // project_id
}
...
const insertSQL = `INSERT INTO "user_projects" ("user_id", "project_id") VALUES ($1, $2) RETURNING "user_id", "project_id"`

const updatePrefix = `UPDATE "user_projects" SET `
const deletePrefix = `DELETE FROM "user_projects"`
...
const insPrefix = "INSERT INTO \"user_projects\""
const insReturning = " RETURNING \"user_id\", \"project_id\""
...
// OnConflictUserIDProjectID upserts on the unique index over (user_id, project_id).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictUserIDProjectID() *Ins {
	n.conflict = 2
	return n
}
...
//	n.OnConflictUserIDProjectID().DoNothing()  // only on that one index
//
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "user_projects", copyCols, src)
}
...
func DeleteOp(userID [16]byte, projectID [16]byte) runtime.BatchOp {
	return runtime.BatchOp{SQL: deleteSQL, Args: []any{userID, projectID}}
}
...
func Delete(ctx context.Context, ex runtime.Executor, userID [16]byte, projectID [16]byte) error {
	n, err := ex.Exec(ctx, deleteSQL, []any{userID, projectID})
	if err != nil {
		return err
	}
	if n == 0 {
