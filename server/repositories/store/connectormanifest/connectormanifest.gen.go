...
// source: table connector_manifests

package connectormanifest
...
func (q Query) DocumentEq(v string) Query        { return q.Where(Document.Eq(v)) }
func (q Query) DocumentNotEq(v string) Query     { return q.Where(Document.NotEq(v)) }
func (q Query) DocumentGt(v string) Query        { return q.Where(Document.Gt(v)) }
func (q Query) DocumentGte(v string) Query       { return q.Where(Document.Gte(v)) }
func (q Query) DocumentLt(v string) Query        { return q.Where(Document.Lt(v)) }
func (q Query) DocumentLte(v string) Query       { return q.Where(Document.Lte(v)) }
func (q Query) DocumentLike(v string) Query      { return q.Where(Document.Like(v)) }
func (q Query) DocumentILike(v string) Query     { return q.Where(Document.ILike(v)) }
func (q Query) DocumentIn(v ...string) Query     { return q.Where(Document.In(v...)) }
func (q Query) DocumentNotIn(v ...string) Query  { return q.Where(Document.NotIn(v...)) }
...
const selectPrefix = `SELECT "id", "created_at", "updated_at", "key", "name", "version", "document", "enabled", "deleted_at" FROM "connector_manifests"`
const countPrefix = `SELECT count(*) FROM "connector_manifests"`
const existsPrefix = `SELECT 1 FROM "connector_manifests"`
...
var orderTable = [nCols][4]string{
	...
	{ // document
		"\"document\"",
		"\"document\" DESC",
		"\"document\" ASC NULLS FIRST",
		"\"document\" DESC NULLS LAST",
	},
	{ // enabled
		"\"enabled\"",
		"\"enabled\" DESC",
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.Key = sl.Str(rv[3])
	r.Name = sl.Str(rv[4])
	r.Version = runtime.Int8(rv[5])
	r.Document = sl.Str(rv[6])
	r.Enabled = runtime.Bool(rv[7])
	r.DeletedAt = runtime.Nullable(rv[8], runtime.Timestamptz)
	return nil
}
...
const insertSQL = `INSERT INTO "connector_manifests" ("id", "created_at", "updated_at", "key", "name", "version", "document", "enabled", "deleted_at") VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING "id", "created_at", "updated_at", "key", "name", "version", "document", "enabled", "deleted_at"`

const updatePrefix = `UPDATE "connector_manifests" SET `
const deletePrefix = `DELETE FROM "connector_manifests"`
...
const (
	...
	dName      uint64 = 1 << 2
	dVersion   uint64 = 1 << 3
	dDocument  uint64 = 1 << 4
	dEnabled   uint64 = 1 << 5
	dDeletedAt uint64 = 1 << 6
)
...
const (
	...
	iName      uint64 = 1 << 4
	iVersion   uint64 = 1 << 5
	iDocument  uint64 = 1 << 6
	iEnabled   uint64 = 1 << 7
	iDeletedAt uint64 = 1 << 8
)
...
const insPrefix = "INSERT INTO \"connector_manifests\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"key\", \"name\", \"version\", \"document\", \"enabled\", \"deleted_at\""
...
func (m *Mut) SetDocument(v string) {
	m.row.Document = v
	m.dirty |= dDocument
}
...
func (n *Ins) SetDocument(v string) {
	n.row.Document = v
	n.set |= iDocument
}
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 6:
			args = append(args, n.row.Document)
		case 7:
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "connector_manifests", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 4:
			args = append(args, m.row.Document)
		case 5:
...
const Table = "connector_manifests"
...
var softDeleteSQL = runtime.SpliceSections(`UPDATE "connector_manifests" SET "deleted_at" = now()`, []runtime.Section{
...
var restoreSQL = runtime.SpliceSections(`UPDATE "connector_manifests" SET "deleted_at" = NULL`, []runtime.Section{
