...
var (
	...
	CreatedAt      = TimeCol{1}
	UpdatedAt      = TimeCol{2}
	OrganizationID = UUIDCol{3}
	Name           = TextCol{4}
	Description    = NullTextCol{5}
	DeletedAt      = NullTimeCol{6}
)
...
const selectPrefix = `SELECT "id", "created_at", "updated_at", "organization_id", "name", "description", "deleted_at" FROM "projects"`
const countPrefix = `SELECT count(*) FROM "projects"`
const existsPrefix = `SELECT 1 FROM "projects"`
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	copy(r.OrganizationID[:], rv[3])
	r.Name = sl.Str(rv[4])
	r.Description = runtime.NullText(rv[5], sl)
	r.DeletedAt = runtime.Nullable(rv[6], runtime.Timestamptz)
	return nil
}
...
// BatchTopByOrganizationID fetches at most n rows for each id, ordered by order.
//
...
func BatchTopByOrganizationID(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return BatchTopByOrganizationIDLateral(ctx, ex, ids, n, order...)
}

// BatchTopByOrganizationIDWindow runs the window lowering.
//
...
// involve writing SQL.
func BatchTopByOrganizationIDWindow(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByOrganizationIDRun(ctx, ex, batchTopByOrganizationIDWindowSQL(order), ids, n)
}

// BatchTopByOrganizationIDLateral runs the lateral lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByOrganizationIDLateral(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByOrganizationIDRun(ctx, ex, batchTopByOrganizationIDLateralSQL(order), ids, n)
}

// batchTopByOrganizationIDRun executes one lowered statement.
func batchTopByOrganizationIDRun(ctx context.Context, ex runtime.Executor, sql string, ids [][16]byte, n int64) ([]Row, error) {
	if len(ids) == 0 || n <= 0 {
		return nil, nil
	}
	if sql == "" {
...
var batchTopByOrganizationIDWindowCache = runtime.NewTreeCache()

// batchTopByOrganizationIDWindowSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByOrganizationIDWindowSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	...
	if st := batchTopByOrganizationIDWindowCache.Get(toks); st != nil {
		return st.SQL
	}
	...
	sql := runtime.SpliceOrder("SELECT \"id\", \"created_at\", \"updated_at\", \"organization_id\", \"name\", \"description\", \"deleted_at\" FROM (SELECT \"id\", \"created_at\", \"updated_at\", \"organization_id\", \"name\", \"description\", \"deleted_at\", row_number() OVER (PARTITION BY \"organization_id\"\x00order\x00) AS \"_storm_rn\" FROM \"projects\" WHERE \"organization_id\" = ANY($1) AND \"deleted_at\" IS NULL) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
	return batchTopByOrganizationIDWindowCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

var batchTopByOrganizationIDLateralCache = runtime.NewTreeCache()

// batchTopByOrganizationIDLateralSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByOrganizationIDLateralSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByOrganizationIDLateralCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"id\", \"_storm_c\".\"created_at\", \"_storm_c\".\"updated_at\", \"_storm_c\".\"organization_id\", \"_storm_c\".\"name\", \"_storm_c\".\"description\", \"_storm_c\".\"deleted_at\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"id\", \"created_at\", \"updated_at\", \"organization_id\", \"name\", \"description\", \"deleted_at\" FROM \"projects\" WHERE \"organization_id\" = \"_storm_p\".\"_storm_k\" AND \"deleted_at\" IS NULL\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
	return batchTopByOrganizationIDLateralCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}
...
const insertSQL = `INSERT INTO "projects" ("id", "created_at", "updated_at", "organization_id", "name", "description", "deleted_at") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING "id", "created_at", "updated_at", "organization_id", "name", "description", "deleted_at"`

const updatePrefix = `UPDATE "projects" SET `
const deletePrefix = `DELETE FROM "projects"`
...
const (
	...
	dOrganizationID uint64 = 1 << 1
	dName           uint64 = 1 << 2
	dDescription    uint64 = 1 << 3
	dDeletedAt      uint64 = 1 << 4
)
...
const insPrefix = "INSERT INTO \"projects\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"organization_id\", \"name\", \"description\", \"deleted_at\""
...
// OnConflictOrganizationIDName upserts on the unique index over (organization_id, name, where deleted_at IS NULL).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictOrganizationIDName() *Ins {
	n.conflict = 4
	return n
}
...
func (s *rowSource) Values() []any {
	...
	s.buf[1] = &r.CreatedAt
	s.buf[2] = &r.UpdatedAt
	s.buf[3] = &r.OrganizationID
	s.buf[4] = &r.Name
	s.buf[5] = r.Description.Ptr()
	s.buf[6] = r.DeletedAt.Ptr()
	return s.buf[:]
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "projects", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 1:
			args = append(args, m.row.OrganizationID)
		case 2:
...
var softDeleteSQL = runtime.SpliceSections(`UPDATE "projects" SET "deleted_at" = now()`, []runtime.Section{
...
var restoreSQL = runtime.SpliceSections(`UPDATE "projects" SET "deleted_at" = NULL`, []runtime.Section{
