...
// source: table variable_snapshots

package variablesnapshot
...
var (
	...
	NodeID     = NullTextCol{4}
	Variables  = JSONCol{5}
	CapturedAt = TimeCol{6}
...
func (q Query) CapturedAtEq(v time.Time) Query            { return q.Where(CapturedAt.Eq(v)) }
func (q Query) CapturedAtNotEq(v time.Time) Query         { return q.Where(CapturedAt.NotEq(v)) }
func (q Query) CapturedAtGt(v time.Time) Query            { return q.Where(CapturedAt.Gt(v)) }
func (q Query) CapturedAtGte(v time.Time) Query           { return q.Where(CapturedAt.Gte(v)) }
func (q Query) CapturedAtLt(v time.Time) Query            { return q.Where(CapturedAt.Lt(v)) }
func (q Query) CapturedAtLte(v time.Time) Query           { return q.Where(CapturedAt.Lte(v)) }
...
const selectPrefix = `SELECT "id", "created_at", "updated_at", "instance_id", "node_id", "variables", "captured_at", "deleted_at" FROM "variable_snapshots"`
const countPrefix = `SELECT count(*) FROM "variable_snapshots"`
const existsPrefix = `SELECT 1 FROM "variable_snapshots"`
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.NodeID = runtime.NullText(rv[4], sl)
	r.Variables = runtime.JSON(runtime.JSONB(rv[5], sl))
	r.CapturedAt = runtime.Timestamptz(rv[6])
...
const insertSQL = `INSERT INTO "variable_snapshots" ("id", "created_at", "updated_at", "instance_id", "node_id", "variables", "captured_at", "deleted_at") VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING "id", "created_at", "updated_at", "instance_id", "node_id", "variables", "captured_at", "deleted_at"`

const updatePrefix = `UPDATE "variable_snapshots" SET `
const deletePrefix = `DELETE FROM "variable_snapshots"`
...
const (
	...
	dNodeID     uint64 = 1 << 2
	dVariables  uint64 = 1 << 3
	dCapturedAt uint64 = 1 << 4
...
const (
	...
	iNodeID     uint64 = 1 << 4
	iVariables  uint64 = 1 << 5
	iCapturedAt uint64 = 1 << 6
...
const insPrefix = "INSERT INTO \"variable_snapshots\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"instance_id\", \"node_id\", \"variables\", \"captured_at\", \"deleted_at\""
...
func (m *Mut) SetCapturedAt(v time.Time) {
	m.row.CapturedAt = v
	m.dirty |= dCapturedAt
}
...
func (n *Ins) SetCapturedAt(v time.Time) {
	n.row.CapturedAt = v
	n.set |= iCapturedAt
}
...
var assignFor = map[string]string{
	...
	"captured_at": "\"captured_at\" = EXCLUDED.\"captured_at\"",
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 6:
			args = append(args, n.row.CapturedAt)
		case 7:
...
func (s *rowSource) Values() []any {
	...
	s.buf[4] = r.NodeID.Ptr()
	s.buf[5] = &r.Variables
	s.buf[6] = &r.CapturedAt
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "variable_snapshots", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 4:
			args = append(args, m.row.CapturedAt)
		case 5:
...
const Table = "variable_snapshots"
...
var softDeleteSQL = runtime.SpliceSections(`UPDATE "variable_snapshots" SET "deleted_at" = now()`, []runtime.Section{
...
var restoreSQL = runtime.SpliceSections(`UPDATE "variable_snapshots" SET "deleted_at" = NULL`, []runtime.Section{
