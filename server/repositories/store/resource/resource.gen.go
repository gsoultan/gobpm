...
// source: table deployment_resources
...
var (
	...
	CreatedAt    = TimeCol{1}
	UpdatedAt    = TimeCol{2}
	DeploymentID = UUIDCol{3}
	Name         = TextCol{4}
	Content      = NullBytesCol{5}
	Type         = TextCol{6}
...
func (q Query) ContentIsNull() Query                  { return q.Where(Content.IsNull()) }
func (q Query) ContentIsNotNull() Query               { return q.Where(Content.IsNotNull()) }
...
const selectPrefix = `SELECT "id", "created_at", "updated_at", "deployment_id", "name", "content", "type", "deleted_at" FROM "deployment_resources"`
const countPrefix = `SELECT count(*) FROM "deployment_resources"`
const existsPrefix = `SELECT 1 FROM "deployment_resources"`
...
var orderTable = [nCols][4]string{
	...
	{ // content
		"\"content\"",
		"\"content\" DESC",
		"\"content\" ASC NULLS FIRST",
		"\"content\" DESC NULLS LAST",
	},
	{ // type
		"\"type\"",
		"\"type\" DESC",
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	copy(r.DeploymentID[:], rv[3])
	r.Name = sl.Str(rv[4])
	r.Content = runtime.Bytes(rv[5])
	r.Type = sl.Str(rv[6])
...
// BatchTopByDeploymentID fetches at most n rows for each id, ordered by order.
//
...
func BatchTopByDeploymentID(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return BatchTopByDeploymentIDLateral(ctx, ex, ids, n, order...)
}

// BatchTopByDeploymentIDWindow runs the window lowering.
//
...
// involve writing SQL.
func BatchTopByDeploymentIDWindow(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByDeploymentIDRun(ctx, ex, batchTopByDeploymentIDWindowSQL(order), ids, n)
}

// BatchTopByDeploymentIDLateral runs the lateral lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByDeploymentIDLateral(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByDeploymentIDRun(ctx, ex, batchTopByDeploymentIDLateralSQL(order), ids, n)
}

// batchTopByDeploymentIDRun executes one lowered statement.
func batchTopByDeploymentIDRun(ctx context.Context, ex runtime.Executor, sql string, ids [][16]byte, n int64) ([]Row, error) {
	if len(ids) == 0 || n <= 0 {
		return nil, nil
	}
	if sql == "" {
...
var batchTopByDeploymentIDWindowCache = runtime.NewTreeCache()

// batchTopByDeploymentIDWindowSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByDeploymentIDWindowSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	...
	if st := batchTopByDeploymentIDWindowCache.Get(toks); st != nil {
		return st.SQL
	}
	...
	sql := runtime.SpliceOrder("SELECT \"id\", \"created_at\", \"updated_at\", \"deployment_id\", \"name\", \"content\", \"type\", \"deleted_at\" FROM (SELECT \"id\", \"created_at\", \"updated_at\", \"deployment_id\", \"name\", \"content\", \"type\", \"deleted_at\", row_number() OVER (PARTITION BY \"deployment_id\"\x00order\x00) AS \"_storm_rn\" FROM \"deployment_resources\" WHERE \"deployment_id\" = ANY($1) AND \"deleted_at\" IS NULL) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
	return batchTopByDeploymentIDWindowCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

var batchTopByDeploymentIDLateralCache = runtime.NewTreeCache()

// batchTopByDeploymentIDLateralSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByDeploymentIDLateralSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByDeploymentIDLateralCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"id\", \"_storm_c\".\"created_at\", \"_storm_c\".\"updated_at\", \"_storm_c\".\"deployment_id\", \"_storm_c\".\"name\", \"_storm_c\".\"content\", \"_storm_c\".\"type\", \"_storm_c\".\"deleted_at\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"id\", \"created_at\", \"updated_at\", \"deployment_id\", \"name\", \"content\", \"type\", \"deleted_at\" FROM \"deployment_resources\" WHERE \"deployment_id\" = \"_storm_p\".\"_storm_k\" AND \"deleted_at\" IS NULL\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
	return batchTopByDeploymentIDLateralCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}
...
const insertSQL = `INSERT INTO "deployment_resources" ("id", "created_at", "updated_at", "deployment_id", "name", "content", "type", "deleted_at") VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING "id", "created_at", "updated_at", "deployment_id", "name", "content", "type", "deleted_at"`

const updatePrefix = `UPDATE "deployment_resources" SET `
const deletePrefix = `DELETE FROM "deployment_resources"`
...
const (
	...
	dDeploymentID uint64 = 1 << 1
	dName         uint64 = 1 << 2
	dContent      uint64 = 1 << 3
	dType         uint64 = 1 << 4
	dDeletedAt    uint64 = 1 << 5
)
...
const (
	...
	iDeploymentID uint64 = 1 << 3
	iName         uint64 = 1 << 4
	iContent      uint64 = 1 << 5
	iType         uint64 = 1 << 6
	iDeletedAt    uint64 = 1 << 7
)
...
const insPrefix = "INSERT INTO \"deployment_resources\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"deployment_id\", \"name\", \"content\", \"type\", \"deleted_at\""
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 3:
			args = append(args, n.row.DeploymentID)
		case 4:
			args = append(args, n.row.Name)
		case 5:
			args = append(args, n.row.Content)
		case 6:
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	...
	args = append(args, r.DeploymentID)
...
func (s *rowSource) Values() []any {
	...
	s.buf[1] = &r.CreatedAt
	s.buf[2] = &r.UpdatedAt
	s.buf[3] = &r.DeploymentID
	s.buf[4] = &r.Name
	s.buf[5] = &r.Content
	s.buf[6] = &r.Type
	s.buf[7] = r.DeletedAt.Ptr()
	return s.buf[:]
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "deployment_resources", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 1:
			args = append(args, m.row.DeploymentID)
		case 2:
			args = append(args, m.row.Name)
		case 3:
			args = append(args, m.row.Content)
		case 4:
...
const Table = "deployment_resources"
...
var softDeleteSQL = runtime.SpliceSections(`UPDATE "deployment_resources" SET "deleted_at" = now()`, []runtime.Section{
...
var restoreSQL = runtime.SpliceSections(`UPDATE "deployment_resources" SET "deleted_at" = NULL`, []runtime.Section{
