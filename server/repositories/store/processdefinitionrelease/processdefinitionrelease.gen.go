...
// source: table process_definition_releases

package processdefinitionrelease
...
var (
	...
	UpdatedAt  = TimeCol{2}
	ProjectID  = UUIDCol{3}
	ProcessKey = TextCol{4}
	ActivateAt = TimeCol{5}
	Version    = Int64Col{6}
...
func (q Query) ProcessKeyEq(v string) Query        { return q.Where(ProcessKey.Eq(v)) }
func (q Query) ProcessKeyNotEq(v string) Query     { return q.Where(ProcessKey.NotEq(v)) }
func (q Query) ProcessKeyGt(v string) Query        { return q.Where(ProcessKey.Gt(v)) }
func (q Query) ProcessKeyGte(v string) Query       { return q.Where(ProcessKey.Gte(v)) }
func (q Query) ProcessKeyLt(v string) Query        { return q.Where(ProcessKey.Lt(v)) }
func (q Query) ProcessKeyLte(v string) Query       { return q.Where(ProcessKey.Lte(v)) }
func (q Query) ProcessKeyLike(v string) Query      { return q.Where(ProcessKey.Like(v)) }
func (q Query) ProcessKeyILike(v string) Query     { return q.Where(ProcessKey.ILike(v)) }
func (q Query) ProcessKeyIn(v ...string) Query     { return q.Where(ProcessKey.In(v...)) }
func (q Query) ProcessKeyNotIn(v ...string) Query  { return q.Where(ProcessKey.NotIn(v...)) }
func (q Query) ActivateAtEq(v time.Time) Query     { return q.Where(ActivateAt.Eq(v)) }
func (q Query) ActivateAtNotEq(v time.Time) Query  { return q.Where(ActivateAt.NotEq(v)) }
func (q Query) ActivateAtGt(v time.Time) Query     { return q.Where(ActivateAt.Gt(v)) }
func (q Query) ActivateAtGte(v time.Time) Query    { return q.Where(ActivateAt.Gte(v)) }
func (q Query) ActivateAtLt(v time.Time) Query     { return q.Where(ActivateAt.Lt(v)) }
func (q Query) ActivateAtLte(v time.Time) Query    { return q.Where(ActivateAt.Lte(v)) }
...
const selectPrefix = `SELECT "id", "created_at", "updated_at", "project_id", "process_key", "activate_at", "version", "deleted_at" FROM "process_definition_releases"`
const countPrefix = `SELECT count(*) FROM "process_definition_releases"`
const existsPrefix = `SELECT 1 FROM "process_definition_releases"`
...
var orderTable = [nCols][4]string{
	...
	{ // process_key
		"\"process_key\"",
		"\"process_key\" DESC",
		"\"process_key\" ASC NULLS FIRST",
		"\"process_key\" DESC NULLS LAST",
	},
	{ // activate_at
		"\"activate_at\"",
		"\"activate_at\" DESC",
		"\"activate_at\" ASC NULLS FIRST",
...
var fragTable = [8][27]runtime.Frag{
	...
	{ // process_key
		...
		{A: "\"process_key\" < $", B: ""},
		{A: "\"process_key\" <= $", B: ""},
		{A: "\"process_key\" LIKE $", B: ""},
		{A: "\"process_key\" ILIKE $", B: ""},
		{},
		{},
		{},
		{},
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.ProcessKey = sl.Str(rv[4])
	r.ActivateAt = runtime.Timestamptz(rv[5])
...
const insertSQL = `INSERT INTO "process_definition_releases" ("id", "created_at", "updated_at", "project_id", "process_key", "activate_at", "version", "deleted_at") VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING "id", "created_at", "updated_at", "project_id", "process_key", "activate_at", "version", "deleted_at"`

const updatePrefix = `UPDATE "process_definition_releases" SET `
const deletePrefix = `DELETE FROM "process_definition_releases"`
...
const (
	...
	dProcessKey uint64 = 1 << 2
	dActivateAt uint64 = 1 << 3
	dVersion    uint64 = 1 << 4
...
const (
	...
	iProcessKey uint64 = 1 << 4
	iActivateAt uint64 = 1 << 5
	iVersion    uint64 = 1 << 6
...
const insPrefix = "INSERT INTO \"process_definition_releases\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"project_id\", \"process_key\", \"activate_at\", \"version\", \"deleted_at\""
...
func (m *Mut) SetProcessKey(v string) {
	m.row.ProcessKey = v
	m.dirty |= dProcessKey
}

func (m *Mut) SetActivateAt(v time.Time) {
	m.row.ActivateAt = v
	m.dirty |= dActivateAt
}
...
func (n *Ins) SetProcessKey(v string) {
	n.row.ProcessKey = v
	n.set |= iProcessKey
}

func (n *Ins) SetActivateAt(v time.Time) {
	n.row.ActivateAt = v
	n.set |= iActivateAt
}
...
var conflictSpecs = []string{
	" ON CONFLICT (\"id\")",
	" ON CONFLICT (\"process_key\", \"project_id\", \"activate_at\") WHERE deleted_at IS NULL",
}
...
func assignable(i uint8, mask uint64) []string {
	...
	case 0:
		...
		if mask&(1<<4) != 0 {
			set = append(set, "process_key")
		}
		if mask&(1<<5) != 0 {
			set = append(set, "activate_at")
		}
		if mask&(1<<6) != 0 {
...
// OnConflictProcessKeyProjectIDActivateAt upserts on the unique index over (process_key, project_id, activate_at, where deleted_at IS NULL).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictProcessKeyProjectIDActivateAt() *Ins {
	n.conflict = 4
	return n
}
...
var assignFor = map[string]string{
	...
	"process_key": "\"process_key\" = EXCLUDED.\"process_key\"",
	"activate_at": "\"activate_at\" = EXCLUDED.\"activate_at\"",
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 4:
			args = append(args, n.row.ProcessKey)
		case 5:
			args = append(args, n.row.ActivateAt)
		case 6:
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	...
	args = append(args, r.ProcessKey)
	args = append(args, r.ActivateAt)
...
func (s *rowSource) Values() []any {
	...
	s.buf[2] = &r.UpdatedAt
	s.buf[3] = &r.ProjectID
	s.buf[4] = &r.ProcessKey
	s.buf[5] = &r.ActivateAt
	s.buf[6] = &r.Version
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "process_definition_releases", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 2:
			args = append(args, m.row.ProcessKey)
		case 3:
			args = append(args, m.row.ActivateAt)
		case 4:
...
const Table = "process_definition_releases"
...
var softDeleteSQL = runtime.SpliceSections(`UPDATE "process_definition_releases" SET "deleted_at" = now()`, []runtime.Section{
...
var restoreSQL = runtime.SpliceSections(`UPDATE "process_definition_releases" SET "deleted_at" = NULL`, []runtime.Section{
