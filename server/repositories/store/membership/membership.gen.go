...
func (q Query) GroupIDEq(v [16]byte) Query       { return q.Where(GroupID.Eq(v)) }
func (q Query) GroupIDNotEq(v [16]byte) Query    { return q.Where(GroupID.NotEq(v)) }
func (q Query) GroupIDIn(v ...[16]byte) Query    { return q.Where(GroupID.In(v...)) }
func (q Query) GroupIDNotIn(v ...[16]byte) Query { return q.Where(GroupID.NotIn(v...)) }

const selectPrefix = `SELECT "user_id", "group_id" FROM "memberships"`
...
var defaultOrder = [2]runtime.Tok{
	...
	runtime.MakeOrder(runtime.Asc, 1), // group_id
}
...
const insertSQL = `INSERT INTO "memberships" ("user_id", "group_id") VALUES ($1, $2) RETURNING "user_id", "group_id"`
...
const insReturning = " RETURNING \"user_id\", \"group_id\""
...
func (n *Ins) SetGroupID(v [16]byte) {
	n.row.GroupID = v
	n.set |= iGroupID
}
...
//	n.OnConflictUserIDGroupID().DoNothing()  // only on that one index
//
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 1:
			args = append(args, n.row.GroupID)
		}
	}
	var out Row
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	args = append(args, m.row.GroupID)
	return runtime.BatchOp{SQL: st.SQL, Args: args}, true
}

// DeleteOp is a delete as a queueable statement.
func DeleteOp(userID [16]byte, groupID [16]byte) runtime.BatchOp {
	return runtime.BatchOp{SQL: deleteSQL, Args: []any{userID, groupID}}
}
...
func Delete(ctx context.Context, ex runtime.Executor, userID [16]byte, groupID [16]byte) error {
	n, err := ex.Exec(ctx, deleteSQL, []any{userID, groupID})
	if err != nil {
		return err
	}
	if n == 0 {
