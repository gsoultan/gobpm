...
// identity is (instance, node, iteration) — the same step of the same instance —
// and IdempotencyKey is what the far end is given so it can recognise the
// repeat too.
//
...
func (s *ServiceCall) Schema(t *storm.Table) {
	...
	// One record per step of an instance: this is what makes a retry find the
	// call rather than make a second one.
	...
	t.UniqueAcrossDeleted(&s.Instance, &s.NodeID, &s.IterationID)
