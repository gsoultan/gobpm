...
func (r *gormUserRepository) Get(ctx context.Context, id uuid.UUID) (models.UserModel, error) {
	...
	if err := MainTx(ctx, r.db).Preload("Organizations").Preload("Projects").First(&m, QueryByID, id).Error; err != nil {
...
func (r *gormUserRepository) GetByUsername(ctx context.Context, username string) (models.UserModel, error) {
	var m models.UserModel
	if err := MainTx(ctx, r.db).Preload("Organizations").Preload("Projects").Where(QueryByUsername, username).First(&m).Error; err != nil {
		return models.UserModel{}, lookupError(err, "user")
	}
	return m, nil
}
...
// tableUsers is the SQL table behind UserModel, needed by name so the tenant
// scope can qualify its columns.
const tableUsers = "users"

// ListByOrganization returns the caller's users, optionally narrowed to one of
// their organizations.
//
// The organization argument is optional and the tenant scope is not. Naming no
// organization used to mean no filter, and the list endpoint accepts an empty
// one — so a caller could read every account in the installation, with the
// preloads below handing over each one's organization and project memberships
// as well. That is the whole directory of an installation, not a count.
func (r *gormUserRepository) ListByOrganization(ctx context.Context, organizationID uuid.UUID) ([]models.UserModel, error) {
	...
	query := tenantScopeMembership(ctx,
		MainTx(ctx, r.db).Model(&models.UserModel{}).Preload("Organizations").Preload("Projects"),
		tableUsers, "user_organizations", "user_model_id")
	if organizationID != uuid.Nil {
		query = query.Where(
			tableUsers+".id IN (SELECT user_model_id FROM user_organizations WHERE organization_model_id = ?)",
			organizationID)
	}
...
func (r *gormUserRepository) Create(ctx context.Context, u models.UserModel, passwordHash string) error {
	...
	if err := MainTx(ctx, r.db).Create(&u).Error; err != nil {
...
func (r *gormUserRepository) Update(ctx context.Context, u models.UserModel) error {
	if err := MainTx(ctx, r.db).Save(&u).Error; err != nil {
...
func (r *gormUserRepository) SetPasswordHash(ctx context.Context, id uuid.UUID, passwordHash string) error {
	...
	result := MainTx(ctx, r.db).Model(&models.UserModel{}).
		Where(QueryByID, id).
...
func (r *gormUserRepository) Delete(ctx context.Context, id uuid.UUID) error {
	if err := MainTx(ctx, r.db).Delete(&models.UserModel{}, QueryByID, id).Error; err != nil {
...
func (r *gormUserRepository) AddOrganization(ctx context.Context, userID, organizationID uuid.UUID) error {
	...
	if err := MainTx(ctx, r.db).Model(&user).Association("Organizations").Append(&org); err != nil {
...
func (r *gormUserRepository) RemoveOrganization(ctx context.Context, userID, organizationID uuid.UUID) error {
	user := models.UserModel{Base: models.Base{ID: models.UUID(userID)}}
	org := models.OrganizationModel{Base: models.Base{ID: models.UUID(organizationID)}}
	if err := MainTx(ctx, r.db).Model(&user).Association("Organizations").Delete(&org); err != nil {
...
func (r *gormUserRepository) AddProject(ctx context.Context, userID, projectID uuid.UUID) error {
	...
	if err := MainTx(ctx, r.db).Model(&user).Association("Projects").Append(&project); err != nil {
...
func (r *gormUserRepository) RemoveProject(ctx context.Context, userID, projectID uuid.UUID) error {
	user := models.UserModel{Base: models.Base{ID: models.UUID(userID)}}
	project := models.ProjectModel{Base: models.Base{ID: models.UUID(projectID)}}
	if err := MainTx(ctx, r.db).Model(&user).Association("Projects").Delete(&project); err != nil {
